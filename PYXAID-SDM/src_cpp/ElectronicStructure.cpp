...
void ElectronicStructure::update_decoherence_times(matrix &rates) {
  ...
  for (int i = 0; i < num_states; i++) {
    ...
    for (int j = 0; j < num_states; j++) {
      tau_m[i] +=
          A->M[j * num_states + j].real() * rates.M[i * num_states + j].real();
    } // for j
  } // for i
}
...
void ElectronicStructure::apply_sdm(double dt, int act_st, matrix &rates,
                                    double tol) {
  if (act_st < 0 || act_st >= num_states) {
    cout << "Error in ElectronicStructure::apply_sdm: active state index out "
            "of range\n";
    exit(0);
  }
  ...
  double p_aa_old = A->M[act_st * num_states + act_st].real();

  if (p_aa_old > 1.0 + tol) {
    double sclf = 1.0 / sqrt(p_aa_old);
    for (int i = 0; i < num_states; i++) {
      Ccurr->M[i] *= sclf;
    }
    update_populations();
    p_aa_old = A->M[act_st * num_states + act_st].real();
  }
  ...
  double inact_st_pop = 0.0;
  for (int i = 0; i < num_states; i++) {
    if (i != act_st) {
      double itau = rates.M[i * num_states + act_st].real();
      double sclf = exp(-dt * itau);
      Ccurr->M[i] *= sclf;

      inact_st_pop += std::norm(Ccurr->M[i]);
    }
  }

  if (inact_st_pop > 1.0) {
    cout << "Error in ElectronicStructure::apply_sdm: inactive-state "
            "population > 1.0\n";
  ...
  if (p_aa_new < 0.0) {
    cout << "Error in ElectronicStructure::apply_sdm: new active-state "
            "population is negative\n";
    cout << "p_aa_new = " << p_aa_new << endl;
    exit(0);
  }

  double sclf = sqrt(p_aa_new / p_aa_old);
  Ccurr->M[act_st] *= sclf;
  ...
  if (fabs(new_norm - 1.0) > 0.1) {
    cout << "Error in ElectronicStructure::apply_sdm: norm deviates too much "
            "from 1\n";
...
void ElectronicStructure::check_decoherence(double dt, int boltz_flag,
                                            double Temp, matrix &rates) {
  ...
  for (int i = 0; i < num_states; i++) {
    ...
    if (t_m[i] >= rnd_i) { // Decoherence event occurs for state i
      ...
      //        if(boltz_flag==1){
      double dE = (Hcurr->M[i * num_states + i] -
                   Hcurr->M[curr_state * num_states + curr_state])
                      .real();
      if (dE > 0) {
        P *= exp(-(dE / (kb * Temp)));
      ...
      if (zeta <
          P) { // Hop to the state i from current state with probability P
        decohere(i);
...
void ElectronicStructure::update_hop_prob(double dt, int boltz_flag,
                                          double Temp, matrix &Ef) {
  /*******************************************************
  ...
  for (int i = 0; i < num_states; i++) {
    ...
    for (int j = 0; j < num_states; j++) {
      if (j != i) {
        ...
        // Hprime* at this moment is -i*hbar*<i|p|j>, Ef will include: 2*e/m_e *
        // A(t) * cos(omega*t)
        ...
        double g_ij = (2.0 * dt / (a_ii * hbar)) *
                      (A->M[i * num_states + j] * Hij).imag(); // g_ij = P(i->j)
...
void ElectronicStructure::update_hop_prob_fssh(double dt, int boltz_flag,
                                               double Temp, matrix &Ef,
                                               double Eex, matrix &rates) {
  /*******************************************************
  ...
  *******************************************************/
  ...
  matrix *Heff;
  Heff = new matrix(num_states, num_states);
  *Heff = *Hcurr +
          (Ef.M[0] * (*Hprimex) + Ef.M[1] * (*Hprimey) + Ef.M[2] * (*Hprimez));
  ...
  for (int i = 0; i < num_states; i++) {
    ...
    for (int j = 0; j < num_states; j++) {
      if (j != i) {
        ...
        g[i * num_states + j] =
            (2.0 * dt / (a_ii * hbar)) *
            (A->M[i * num_states + j] * Heff->M[i * num_states + j])
                .imag(); // g_ij = P(i->j)
...
void ElectronicStructure::update_hop_prob_mssh(double dt, int boltz_flag,
                                               double Temp, matrix &Ef,
                                               double Eex, matrix &rates) {
  /*******************************************************
    Here we actually sum up all the transition probabilities
  ...
  *Heff = *Hcurr +
  ...
  for (int i = 0; i < num_states; i++) {
    ...
    for (int j = 0; j < num_states; j++) {
      if (j != i) {
        g[i * num_states + j] =
            A->M[j * num_states + j].real(); // g_ij = P(i->j)
...
void ElectronicStructure::update_hop_prob_gfsh(double dt, int boltz_flag,
                                               double Temp, matrix &Ef,
                                               double Eex, matrix &rates) {
  /*******************************************************
   Here we actually sum up all the transition probabilities
  ...
  *C_dot = -one * (*Heff * *Ccurr); // assume hbar = 1
  ...
  matrix *A_dot;
  A_dot = new matrix(num_states, num_states);
    ...
    for (j = 0; j < num_states; j++) {
      ...
      if (j != i) { // off-diagonal = probabilities to hop to other states
        ...
        if (a[i] < 1e-12) {
          ...
          if (g[i * num_states + j] <
              0.0) { // since norm is negative, than this condition means that
                     // a_dot[i] and a_dot[j] have same signs which is bad - so
                     // no transitions are assigned
            g[i * num_states + j] = 0.0;
          } else { // here we have opposite signs of a_dot[i] and a_dot[j], but
...
void ElectronicStructure::rot(complex<double> Hij, double dt, int i, int j) {
  /***********************************************************************
    Action of operator: exp(iL_ij*dt) = exp(-(i/hbar)*dt*(H_ij*c_j*d/dc_i +
  H_ji*c_i*d/dc_j)):
...
void ElectronicStructure::propagate_coefficients1(double dt, int opt,
                                                  matrix &Ef) {
  /***********************************************************************
