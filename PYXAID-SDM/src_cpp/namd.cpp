...
/*****************************************************************
  ...
  void regression(vector<double>& X,vector<double>& Y,int opt,double& a,double&
b) double decoherence_rates(vector<double>& x,double dt,std::string rt_dir,int
regress_mode) void Efield(InputStructure& is,double t,matrix& E,double& Eex)
  void propagate_electronic(InputStructure& is,vector<ElectronicStructure>&
es,int i, matrix& rates) void solve_electronic(InputStructure&
is,vector<ElectronicStructure>& es,matrix& rates) void
run_decoherence_rates(InputStructure& is, vector<ElectronicStructure>&
me_es,vector<me_state>& me_states, int icond) void run_namd(InputStructure& is,
vector<ElectronicStructure>& me_es,vector<me_state>& me_states, int icond) void
run_namd1(InputStructure& is, vector<ElectronicStructure>&
me_es,vector<me_state>& me_states, int icond)
...
void hop(vector<double> &sh_prob, int &hopstate, int numstates) {
  ...
  ************************************************/
  ...
  int hstate = -1;   // set to an absurd value, so that run fails explicitly if
                   // the surface hopping probabilities are stange
...
void regression(vector<double> &X, vector<double> &Y, int opt, double &a,
                double &b) {
  // Linear regression
  // opt = 0:   Y =     b*X
  // opt = 1:   Y = a + b*X
  if (X.size() != Y.size()) {
    cout
        << "Error in regression: Size of X array is different from that of Y\n";
    exit(0);
  }
...
double decoherence_rates(vector<double> &x, double dt, std::string rt_dir,
                         int regress_mode) {
  /***********************************************
   Computes:
  ...
  ***********************************************/
  ...
  double dE = 0.0025;    // spacing for x (energy) axis for spectral density
                         // function = 20 cm^-1
  ...
  for (w = 0; w < Npoints; w++) {
    out1 << "w(eV)= " << w * dE << " w(cm^-1)= " << w * dE * 8065.54468111324
         << " J= " << J[w] << " sqrt(J)= " << sqrt(J[w]) << endl;
  }
  out1.close();
...
void Efield(InputStructure &is, double t, matrix &E, double &Eex) {
  ...
  if (is.is_field == 1) {
    ...
    if (is.field_dir == "x") {
      ...
      cout << "Value " << is.field_dir
           << " for the field_dir variable is unknown. Exiting...\n";
      exit(0);
    }
    ...
    Em ^
      ...
      *******************************/
      ...
      } else { // Not defined - then constant for whole time period of
               // simulation
        Em = 1.0;
      }
    } // protocol == 1
    ...
    if (is.is_field_fluence) {
      if (is.field_protocol == 1) {
        Ampl = 0.01038 * lambda *
               sqrt(is.field_fluence / is.field_T); // result is in eV*Bohr
      } // protocol==1
      else if (is.field_protocol == 2) {
        Ampl = 0.06857 * sqrt(is.field_fluence * lambda /
                              fabs(sin(2.0 * omega * is.field_Tm) -
                                   cos(2.0 * omega * is.field_Tm)));
      } // protocol==2
...
void propagate_electronic(InputStructure &is, vector<ElectronicStructure> &es,
                          int i, matrix &rates) {

  int nel =
      is.nucl_dt / is.elec_dt; // Number of electronic iterations per 1 nuclear
  ...
  if (is.integrator == 0) {
    for (int j = 0; j < nel; j++) {
      ...
      if (is.sh_algo == 0) {
        es[i].update_hop_prob_fssh(is.elec_dt, is.boltz_flag, is.Temp, Ef, Eex,
                                   rates);
      } else if (is.sh_algo == 1) {
        es[i].update_hop_prob_gfsh(is.elec_dt, is.boltz_flag, is.Temp, Ef, Eex,
                                   rates);
      } else if (is.sh_algo == 2) {
        es[i].update_hop_prob_mssh(is.elec_dt, is.boltz_flag, is.Temp, Ef, Eex,
                                   rates);
      }
...
void solve_electronic(InputStructure &is, vector<ElectronicStructure> &es,
  ...
  //>>>>> Stage 1: Propagate orbitals (coefficients) and compute hopping
  //probabilities
  for (int i = 0; i < sz; i++) { // Nuclear iterations - MD trajectory length
    ...
    es[i].init_hop_prob1();
    propagate_electronic(is, es, i,
                         rates); // it also updates the hopping probabilities
    ...
    */
    ...
    if (is.debug_flag == 1) {
      ...
      cout << "After namd nuclear iteration " << i
           << " populations of all considered states are:" << endl;
      for (int j = 0; j < es[i].num_states; j++) {
        for (int k = 0; k < es[i].num_states; k++) {
          cout << "a(" << j << "," << k
               << ") = " << es[i].A->M[j * es[i].num_states + k].real() << " + "
               << es[i].A->M[j * es[i].num_states + k].imag() << "i" << " ";
        }
        cout << endl;
      }
      cout << "Hopping probabilities:\n";
      for (j = 0; j < es[i].num_states; j++) {
        cout << "P( " << es[i].curr_state << " --> " << j
             << " )= " << setprecision(10)
      ...
      double norm = 0.0;
      for (j = 0; j < es[i].num_states; j++) {
        cout << "c[" << j << "] = " << es[i].Ccurr->M[j].real() << " + "
             << es[i].Ccurr->M[j].imag() << "i \n";
...
void run_decoherence_rates(InputStructure &is,
                           ...
                           vector<me_state> &me_states, int icond) {
  ...
  matrix rij(N, N);
  ofstream out(
      (is.scratch_dir + "/decoherence_rates_icond" + int2string(icond) + ".txt")
          .c_str(),
      ios::out);
  ...
  for (int i = 0; i < N; i++) {
    for (int j = 0; j < N; j++) {
      if (i == j) {
        ...
        // First lets extract the energy differences of the levels i and j along
        // the trajectory
        vector<double> Eij(sz, 0.0);
        double dEij, ave_dEij;
        ave_dEij = 0.0;
        for (int t = 0; t < sz; t++) {
          dEij = me_es[t].Hcurr->M[i * N + i].real() -
                 me_es[t].Hcurr->M[j * N + j].real();
          Eij[t] = dEij;
          ave_dEij += dEij;
        }
        ave_dEij /= ((double)sz);
        ...
        rij.M[i * N + j] =
            decoherence_rates(Eij, is.nucl_dt,
                              is.scratch_dir + "/icond" + int2string(icond) +
                                  "pair" + int2string(i) + "_" + int2string(j),
                              is.regress_mode);
      }
...
void run_namd(InputStructure &is, vector<ElectronicStructure> &me_es,
              vector<me_state> &me_states, int icond) {
  ...
  //=====================================
  solve_electronic(is, me_es, rates); // rates  are not actually used, because
                                      // this is non-decoherence algorithm
  //}// algorithm = 0
  ...
  for (int i = 0; i < sz; i++) {
    ...
    for (int j = 0; j < me_es[i].num_states; j++) {
      out << "P(" << j << ")= " << setprecision(10)
          << me_es[i].A->M[j * me_es[i].num_states + j].real() << "  ";
  ...
  //>>>>> Stage 2: Now we just run the random walk (surface hopping) with
...
void run_namd1(InputStructure &is, vector<ElectronicStructure> &me_es,
               vector<me_state> &me_states, int icond) {
  // This version is different from run_namd function in that it does not
  // separate solving TD-SE and computation of the surface hopping
  // probabilities. This is because here we inlcude decoherence effects, which
  // effectively modify wavefunction (TD-SE solution) along the trajectories
  // stochastically, so it is not possible to separate.
  ...
  vector<vector<double>> z(
      nst, std::vector<double>(
               nst, 0.0)); // 2D matrix with all components set to 0.0

  vector<vector<double>> E0(nst, vector<double>(nst, 0.0)); // average
  vector<vector<double>> d2E_av(
      nst, vector<double>(nst, 0.0)); // average fluctuation of i-j pair
  vector<vector<vector<double>>> d2E(
      sz, vector<vector<double>>(nst, vector<double>(nst, 0.0))); // fluctuation
  matrix rates(nst, nst);

  if (is.decoherence > 0) {
    cout << "Reading decoherence rate matrix for this initial condition...\n";
    std::string filename = is.scratch_dir + "/decoherence_rates_icond" +
                           int2string(icond) + ".txt";
    ...
    if (is.decoherence == 2) { // NAC scaling
      ...
      std::string filename = is.scratch_dir + "/scaling_factors_icond" +
                             int2string(icond) + ".txt";
      ...
      for (i = 0; i < nst; i++) {
        for (j = 0; j < nst; j++) {
          for (t = 0; t < sz; t++) {
            E0[i][j] += (me_es[t].Hcurr->M[i * nst + i].real() -
                         me_es[t].Hcurr->M[j * nst + j].real());
          } // for t
          E0[i][j] /= ((double)sz);
        } // for j
      } // for i
      ...
      for (i = 0; i < nst; i++) {
        for (j = 0; j < nst; j++) {
          for (t = 0; t < sz; t++) {
            double de = ((me_es[t].Hcurr->M[i * nst + i].real() -
                          me_es[t].Hcurr->M[j * nst + j].real()) -
                         E0[i][j]);
      ...
      for (t = 0; t < sz; t++) {
        ...
        for (i = 0; i < nst; i++) {
          for (j = 0; j < nst; j++) {
            if (i != j) {
              // My original version
              //              double dEij = (me_es[t].Hcurr->M[i*nst+i].real() -
              //              me_es[t].Hcurr->M[j*nst+j].real()) - E0[i][j];
      ...
      for (i = 0; i < nst; i++) {
        for (j = 0; j < nst; j++) {
          if (i != j) {
            ...
            std::string filename = is.scratch_dir + "/icond" +
                                   int2string(icond) + "pair" + int2string(i) +
                                   "_" + int2string(j) + "Spectral_density.txt";
            cout << "Expected filename is: " << filename << endl;
            ...
            // Reading spectral density for given pair, storing data in arrays G
            // (gaps) and J (spectral dens.)
            ...
            // Now we are ready to scale the gap for i->j transition for all
            // times
            for (t = 0; t < sz; t++) {
              out << "t= " << t << "  ";
              ...
              //              double scl = J[indx] + fra*(J[indx+1] -
              //              J[indx])/(G[indx+1] - G[indx]); double scl =
              //              J[indx]/sumJ;  // density of vibronic states at
              //              given gap
    ...
    if (is.decoherence ==
        4) { // NAC scaling (decoherence==2) but with extrapolation
      ...
      /*
            std::string filename =
         is.scratch_dir+"/scaling_factors_icond"+int2string(icond)+".txt";
            ofstream out(filename.c_str(),ios::out);
            ...
            for(int t=0;t<sz;t++){
              ...
              for(int i=0;i<nst;i++){
                for(int j=0;j<nst;j++){
                  if(i!=j){
                    double dEij = (me_es[t].Hcurr->M[i*nst+i].real() -
         me_es[t].Hcurr->M[j*nst+j].real()); double tau = 1000.0; // 1 ps
                    if(rates.M[i*nst+j].real()>0.0){
                      tau = (1.0/rates.M[i*nst+j].real());
                    }
    ...
    if (is.decoherence == 6) {
      // nothing special:  Reserved for FSSH with wfc collapse - which works
      // fine for ECWR
    }

    if (is.decoherence ==
        7) { // Here goes the NAC scaling that is based on FT and FC factors
             /*
                   std::string filename =
                is.scratch_dir+"/scaling_factors_icond"+int2string(icond)+".txt";
                   ofstream out(filename.c_str(),ios::out);
  ...
  // Because we will be propagating num_sh_traj independent trajectories at the
  // same time, we will need such an array, initially containing identical
  // entries. Unfortunately this means the memory requirement will increase
  // num_sh_traj_times (hopefully in the future I will optimize such a
  // structure, because the Hamiltonian(biggest chunk of the memory requirement)
  // is the same everywhere), but for now will do kinda brute force
  /*
  ...
  */
  ...
  // The initial state and coefficients remain unchanged, so we can use the
  // me_es strait away Only we need to make coupule 2D arrays to store results.

  //==================== Propagate many-electron orbitals
  //=====================================
  // The outer loop (which calls run_namd1 function) averages over initial
  // conditions
  ...
  for (n = 0; n < is.num_sh_traj; n++) {
    ...
    for (i = 0; i < sz; i++) {
      ...
      propagate_electronic(
          is, me_es, i,
          rates); // update_hop_prob -is called in there
      ...
      else if (is.decoherence == 2 || is.decoherence == 3 ||
               is.decoherence == 4) { // NAC scaling
        //        curr_state = me_es[i].curr_state;
        hop(me_es[i].g, me_es[i].curr_state, nst);
        curr_state = me_es[i].curr_state;
      ...
      else if (is.decoherence == 8) { // SDM
        hop(me_es[i].g, me_es[i].curr_state, nst);
        curr_state = me_es[i].curr_state;

        me_es[i].apply_sdm(is.nucl_dt, curr_state, rates);
        me_es[i].update_populations();
      } // is.decoherence==8
