...
`PYXAID`, developed by `Oleg V. Prezhdo` and `Alexey V. Akimov`, is a well-established nonadiabatic molecular dynamics software widely used for excited-state simulations in condensed matter systems. Subsequently, `Libra` developed by `Alexey V. Akimov` further extended the methodological framework. In addition, other software packages, such as `Hefei-NAMD`, `NEXMD`, `SHARC`, `Newton-X`, and `SPADE` have also been widely applied in excited-state dynamics studies and demonstrated strong performance.
