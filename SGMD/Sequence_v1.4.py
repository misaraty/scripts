...
def resolve_target_direction(values):
    ...
    if mode in {"max", "maximize", "high", "higher"}:
        RESOLVED_DIRECTION = "max"
    elif mode in {"min", "minimize", "low", "lower"}:
        RESOLVED_DIRECTION = "min"
    elif mode == "auto":
        ...
        raise ValueError("TARGET_DIRECTION must be auto/max/min")
...
def extract_top_seeds(pred_csv, topk=TOPK_SEED):
...
def enrich_by_predictions(ckpt, gen_csv, tag, topk=TOPK_SEED):
    ...
    seeds = extract_top_seeds(preds, topk=topk) if preds else []
    return preds, seeds
...
def select_seed_smiles(seed_smiles, topk, num_gen, round_id):
    if not seed_smiles:
        return []
    uniq = []
    seen = set()
    for s in seed_smiles:
        ...
        if cs is not None and cs not in seen:
            seen.add(cs)
            uniq.append(cs)
    k = min(len(uniq), int(topk), int(num_gen))
    if k <= 0:
        return []
...
def _sample_in_batches(
    ...
    selected = select_seed_smiles(seed_smiles, TOPK_SEED, num_gen, round_id)
    all_sequences = []
    generated = 0
    seed_offset = 0
...
def main():
    ...
    seeds = extract_top_seeds(merged_pred, TOPK_SEED)
