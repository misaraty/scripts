...
def resolve_target_direction(df):
    ...
    mode = str(TARGET_DIRECTION).strip().lower()
    aliases = {
        "max": "max",
        "maximize": "max",
        "higher": "max",
    ...
    if mode not in aliases:
        raise ValueError("TARGET_DIRECTION must be auto, max, or min")
    mode = aliases[mode]
    median_d = float(pd.to_numeric(df["D"], errors="coerce").median())
    if mode == "auto":
        mode = "max" if float(TARGET_D) >= median_d else "min"
    TARGET_DIRECTION_RESOLVED = mode
    print(
...
def initialize_chemical_space(df):
    ...
    if not elements or not heavy_counts:
    ...
    s = pd.Series(heavy_counts, dtype=float)
    DM_HEAVY_ATOM_MIN = int(math.floor(float(s.quantile(qlow))))
    DM_HEAVY_ATOM_MAX = int(math.ceil(float(s.quantile(qhigh))))
    DM_HEAVY_ATOM_MIN = max(1, DM_HEAVY_ATOM_MIN)
...
def main():
    ...
    chemprop_predict_paths(ckpt, merged_prev, "MERGED_R0")
    ...
    for r in range(1, ENRICH_ROUNDS + 1):
        ...
        chemprop_predict_paths(ckpt, merged, f"MERGED_R{r}")
        merged_prev = merged
