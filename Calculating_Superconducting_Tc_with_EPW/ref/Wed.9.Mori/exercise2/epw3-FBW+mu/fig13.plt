...
plot \
for [temp in "10 15 20 25 30 35 40 45 50 55"] gprintf('../epw2-FBW/mgb2.imag_aniso_gap0_%06.2f',temp) u (($3)+scale*($5)):($2) with filledc above x1=temp lc rgb "red" lw 0 notitle, \
for [temp in "10"] gprintf('../epw2-FBW/mgb2.imag_aniso_gap0_%06.2f',temp) u (($3)+scale*($5)):($2) w l lc rgb "red" dt 3 lw 2 title "fixed μ_F", \
for [temp in "15 20 25 30 35 40 45 50 55"] gprintf('../epw2-FBW/mgb2.imag_aniso_gap0_%06.2f',temp) u (($3)+scale*($5)):($2) w l lc rgb "red" dt 3 lw 2 notitle, \
...
for [temp in "10"] gprintf('./mgb2.imag_aniso_gap0_%06.2f',temp) u (($3)+scale*($5)):($2) w l lc rgb "blue" lw 1 title "variable μ_F", \
